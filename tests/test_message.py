...
class MessageTestCase(unittest.TestCase):
    def test_pack_message_type(self):
        ...
        m = message.Message(message.MessageClass.INDICATION, message.MessageMethod.BINDING)
        ...
        m = message.Message(message.MessageClass.ERROR_RESPONSE, message.MessageMethod.BINDING)
        self.assertEqual(m._pack_message_type(), 0b000100010001)

        m = message.Message(message.MessageClass.REQUEST, message.MessageMethod.CONNECTION_BIND)
        self.assertEqual(m._pack_message_type(), 0b000000001011)

        m = message.Message(message.MessageClass.INDICATION, message.MessageMethod.CONNECTION_BIND)
        self.assertEqual(m._pack_message_type(), 0b000000011011)

        m = message.Message(message.MessageClass.SUCCESS_RESPONSE, message.MessageMethod.CONNECTION_BIND)
        self.assertEqual(m._pack_message_type(), 0b000100001011)

        m = message.Message(message.MessageClass.ERROR_RESPONSE, message.MessageMethod.CONNECTION_BIND)
        self.assertEqual(m._pack_message_type(), 0b000100011011)

        m = message.Message(message.MessageClass.REQUEST, message.MessageMethod.GOOG_PING)
        self.assertEqual(m._pack_message_type(), 0b001000000000)

        m = message.Message(message.MessageClass.INDICATION, message.MessageMethod.GOOG_PING)
        self.assertEqual(m._pack_message_type(), 0b001000010000)

        m = message.Message(message.MessageClass.SUCCESS_RESPONSE, message.MessageMethod.GOOG_PING)
        self.assertEqual(m._pack_message_type(), 0b001100000000)

        m = message.Message(message.MessageClass.ERROR_RESPONSE, message.MessageMethod.GOOG_PING)
    ...
    def test_packed_length_with_attributes(self):
        ...
        m = message.Message(message.MessageClass.REQUEST, message.MessageMethod.BINDING, attributes=[una, fa])
    ...
    def test_pack_no_attributes(self):
        m = message.Message(message.MessageClass.REQUEST, message.MessageMethod.BINDING, b'\x01' * 12)
        self.assertEqual(m.pack(), b'\x00\x01\x00\x00\x21\x12\xA4\x42' + b'\x01' * 12)

    def test_pack_with_attributes(self):
        una = UsernameAttribute(b'123')
        fa = FingerprintAttribute(0xA5A5A5A5)

        m = message.Message(message.MessageClass.REQUEST, message.MessageMethod.BINDING, b'\x01' * 12, [una, fa])
        self.assertEqual(m.pack(), b'\x00\x01\x00\x10\x21\x12\xA4\x42' + b'\x01' * 12 + una.pack() + fa.pack())

    def test_unpack_no_attributes(self):
        m1 = message.Message(message.MessageClass.REQUEST, message.MessageMethod.BINDING)
        m2 = message.Message(message.MessageClass.INDICATION, message.MessageMethod.CHANNEL_BIND)
    ...
    def test_unpack_with_attributes(self):
        ...
        m1 = message.Message(message.MessageClass.REQUEST, message.MessageMethod.BINDING, b'\x01' * 12, [una, fa])
        m2 = message.Message(message.MessageClass.INDICATION, message.MessageMethod.CHANNEL_BIND)
