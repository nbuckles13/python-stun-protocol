...
class Message():
    ...
    def _unpack_message_type(self, message_type) -> None:
        # message class is encoded into the 11th and 7th bits
        self.message_class = ((message_type & 0x0100) >> 7) | (
            (message_type & 0x0010) >> 4)
    ...
    def unpack(self, buffer: bytes, unpack_undefined_attributes: bool = False):
        (message_type, message_length, _, self.transaction_id) = struct.unpack(
            self.STRUCT_HEADER_FORMAT, buffer[:20])
        ...
        while offset < remaining_length:
            a = attribute.create(
                remaining_buffer[offset:], unpack_undefined_attributes)

            self.attributes.append(a)
            offset += a.packed_length()

    @classmethod
    def create(cls: 'Message', buffer: bytes, unpack_undefined_attributes: bool = False) -> 'Message':
        m = Message(MessageClass.REQUEST, MessageMethod.BINDING)
        m.unpack(buffer, unpack_undefined_attributes)
        return m
    ...
    def add_message_integrity_attribute(self, key: bytes) -> None:
        ...
        mia = MessageIntegrityAttribute(
            b'0' * MessageIntegrityAttribute.fixed_length())
        ...
        mia_value = hmac.new(
            key, packed_message[0:-mia.packed_length()], sha1).digest()
    ...
    def add_message_integrity_sha256_attribute(self, key: bytes) -> None:
        ...
        mia = MessageIntegritySha256Attribute(
            b'0' * MessageIntegritySha256Attribute.maximum_length())
        self.attributes.append(mia)
        ...
        mia_value = hmac.new(
            key, packed_message[0:-mia.packed_length()], sha256).digest()
        self.attributes[-1].hmac = mia_value
    ...
    def add_fingerprint_attribute(self) -> None:
        ...
        fpa_value = binascii.crc32(
            packed_message[0:-fpa.packed_length()], 0) ^ 0x5354554e
    ...
    def add_xor_mapped_address_attribute_v4(self, port: int, address: int) -> None:
        ...
        xmaa = XorMappedAddressAttribute(
            MappedAddressAttributeBase.family_ipv4, port, address)
